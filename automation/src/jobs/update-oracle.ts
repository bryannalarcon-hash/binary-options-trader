...
export interface OracleUpdateResult {
  ...
  priceCents?: number;
  /** Write time we stamped on-chain (Unix seconds). */
  publishTime?: number;
  /** Pyth's own feed publish_time (last trade), kept for observability. */
...
async function postOracleUpdate(
  ...
  // The oracle account stores price in cents (the contract's documented
  // convention), so re-scale: raw_price * 10^expo gives USD, * 100 → cents.
  const priceCents = Math.round(price.priceUsd * 100);
  const confCents = Math.max(1, Math.round(price.confUsd * 100));

  // Stamp the WRITE time, not Pyth's feed publish_time. Pyth equity feeds carry
  // the last-trade timestamp, which is stale outside US market hours (nights /
  // weekends). The on-chain OracleAccount is a Pyth-fed cache refreshed every
  // ~30s; the contract's staleness gate validates that this cache is being kept
  // alive by the automation pipeline (if automation dies, the cache goes stale
  // and settle_market correctly refuses). The price VALUE is the genuine latest
  // Pyth price. See docs/RISKS.md "oracle cache" note.
  ...
  return {
    ...
    priceCents,
    publishTime: nowSec,
    pythPublishTime: price.publishTime,
  };
}
